...
public class Line {
	...
	public Line(String linevalue) {
		...
		if (!label.equals("") && !opcode.equalsIgnoreCase("EQU")) {
	...
	public void spaceCheck() {
		if ((linevalue.charAt(0) == ' ') && !label.trim().equals("")) {
			...
			ErrorHandle.getError(2);
				...
				&& (linevalue.length() < 17 || !((linevalue.charAt(15) == ' ') || (linevalue.charAt(16) == ' ')))) {
			isCorrect = false;
	...
	public void opcodeCheck() {
		...
		if (operation == null) {
			...
			return;
		} else if ((operation.op.equalsIgnoreCase("start") || operation.op.equalsIgnoreCase("end")
	...
	public void splitLine() {
		...
		if (linevalue.length() > 16) {
			...
			} else if (opcode.equalsIgnoreCase("WORD") || opcode.equalsIgnoreCase("RESW")
	...
	public void equCheck() {
		...
		if (!label.equals("")) {
			...
			if (operand1.contains("+")) {
				...
				operand1 = arr[0];
				operand2 = arr[1];
				// operand1 not already declared
				if (!Data.symTab.containsKey(operand1) || Data.symTab.get(operand1).adress == -1) {

					ErrorHandle.getError(9);
					isCorrect = false;
					return;
				}
				// operand2 not already declared
				if (!Data.symTab.containsKey(operand2) || Data.symTab.get(operand2).adress == -1) {
					try {
						// check if operand2 is an int
						value = Integer.parseInt(operand2);
						Data.addEQU(label, (Data.symTab.get(operand1).adress) + value);
					} catch (Exception e) {
						ErrorHandle.getError(9);
						isCorrect = false;
						return;
					...
					// operand1 and operand2 are both declared
					Data.addEQU(label, (Data.symTab.get(operand1).adress) + (Data.symTab.get(operand2).adress));
				return;
				...
				operand1 = arr[0];
				operand2 = arr[1];
				// operand1 is not already declared
				if (!Data.symTab.containsKey(operand1) || Data.symTab.get(operand1).adress == -1) {
					ErrorHandle.getError(9);
					isCorrect = false;
					return;
	...
	public void operandCheck() {
					...
					if (!operation.type.equals(Table.storage)) {
						...
						if (!opcode.equals("EQU"))
							if (!Data.addSymbol(operand1, -1)) {
								System.out.println("hey why duplicate");
								isCorrect = false;
								return;
							}
					}
		...
		if ((Data.registers.containsKey(operand1.toUpperCase()) == false)) {
			if (operand1.startsWith("#")) {
				...
				addressingmode = 0;
				// check if !is number check present in symtb if format 3 || regtable if format
				// 2or3
			} else if (operand1.startsWith("@")) {
				String[] splitted1 = operand1.split("@");
				operand1 = splitted1[1];
